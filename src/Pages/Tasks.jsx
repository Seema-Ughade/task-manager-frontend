...
const Tasks = () => {
  ...
  const [tagaddTask, settagAddTask] = useState({ tags: '' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //tags
        const Tagsresponse = await axios.get('https://task-manager-backend-1-3zvs.onrender.com/api/tags');
        setTags(Tagsresponse.data); // Assuming the response is an array of tags
  ...
  const handleInputChange = (e) => {
    ...
    if (e.target) {
      ...
      settagAddTask({ ...addTask, [name]: value });
  ...
  <form onSubmit={handleAddTask} className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      ...
      >
        ...
        {tags.map((tag) => (
          <option key={tag.id} value={tag.name}>{tag.name}</option> // Adjust key and value based on your API response structure
        ))}
      </select>
    </div>
    <div>
