...
const ActivityTypes = () => {
  ...
  useEffect(() => {
    const fetchActivityTypes = async () => {
      try {
        ...
        const response = await axios.get('http://127.0.0.1:5000/api/activity-types');
  ...
  const handleDeleteClick = async (id) => {
    if (window.confirm('Are you sure you want to delete this activity type?')) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/activity-types/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://127.0.0.1:5000/api/activity-types/${editId}`, newActivityType);
        setActivityTypes(activityTypes.map((activityType) => (activityType._id === editId ? { ...activityType, ...newActivityType } : activityType)));
      } else {
        const response = await axios.post('http://127.0.0.1:5000/api/activity-types', newActivityType);
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      const promises = activityTypesArray.map(activityType => axios.post('http://127.0.0.1:5000/api/activity-types', { name: activityType }));
