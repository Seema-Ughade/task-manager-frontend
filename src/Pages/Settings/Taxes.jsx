...
const Taxes = () => {
  ...
  useEffect(() => {
    const fetchTaxes = async () => {
      try {
        setLoading(true);
        const response = await axios.get('http://127.0.0.1:5000/api/taxes');
  ...
  const handleDeleteClick = async (id) => {
    if (window.confirm('Are you sure you want to delete this tax type?')) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/taxes/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://127.0.0.1:5000/api/taxes/${editId}`, newTax);
        setTaxes(taxes.map((tax) => (tax._id === editId ? { ...tax, ...newTax } : tax)));
      } else {
        const response = await axios.post('http://127.0.0.1:5000/api/taxes', newTax);
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      const promises = taxesArray.map(tax => {
        ...
        return axios.post('http://127.0.0.1:5000/api/taxes', { name: name.trim(), rate: parseFloat(rate.trim()) });
      });
