...
const Tags = () => {
  ...
  const handleDeleteClick = async (id) => {
    if (window.confirm('Are you sure you want to delete this tag?')) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/tags/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://127.0.0.1:5000/api/tags/${editId}`, newTag);
        setTags(tags.map((tag) => (tag._id === editId ? { ...tag, ...newTag } : tag)));
      } else {
        const response = await axios.post('http://127.0.0.1:5000/api/tags', newTag);
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      const promises = tagsArray.map(tag => axios.post('http://127.0.0.1:5000/api/tags', { name: tag }));
