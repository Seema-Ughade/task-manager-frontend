...
const Status = () => {
  ...
  useEffect(() => {
    const fetchStatuses = async () => {
      try {
        ...
        const response = await axios.get('http://127.0.0.1:5000/api/statuses');
  ...
  const handleDeleteClick = async (id) => {
    if (window.confirm('Are you sure you want to delete this status?')) {
      try {
        await axios.delete(`http://127.0.0.1:5000/api/statuses/${id}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await axios.put(`http://127.0.0.1:5000/api/statuses/${editId}`, newStatus);
        setStatuses(
          statuses.map((status) =>
        ...
        const response = await axios.post('http://localhost:5000/api/statuses', newStatus);
